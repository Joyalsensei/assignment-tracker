...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS assignments 
                   (id INTEGER PRIMARY KEY, student_name TEXT, name TEXT, due_date TEXT, subject TEXT)''')
    conn.commit()
    conn.close()
...
def tracker():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        conn.execute('INSERT INTO assignments (student_name, name, due_date, subject) VALUES (?, ?, ?, ?)', 
